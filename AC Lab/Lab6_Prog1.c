...
struct node* bin_HEAP_EXTRACT_MIN(struct node* H1)
{
    ...
    //x points to current min node..p is used to traverse the list to find min value in its sibling..once sibling found , assign x to p's sibling..t holds previous node of x
    min=x->n;
    p = x;
    ...
    if (t == NULL && x->sibling == NULL)
        H1 = NULL; //only one node
    else if (t == NULL)
        H1 = x->sibling; //2 nodes and no child for node 1
    else if (t->sibling == NULL)
        t = NULL; //dk
    else
        t->sibling = x->sibling; // x removed
    if (x->child != NULL)
    {
        Hr = REVERT_LIST(x->child);
...
int bin_HEAP_DECREASE_KEY(struct node* H, int i, int k)
{
    ...
    if (k > p->n)
    {
        printf("\nTHE NEW KEY IS GREATER THAN CURRENT ONE");
        return 0;
    }
    p->n = k;
    y = p;
    ...
    do
    {
        ...
        switch (l)
        {
            ...
            case 5:
                ...
                printf("%d is minimum",minimumNode->n);
